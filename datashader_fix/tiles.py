...
def calculate_zoom_level_stats(full_extent, level, load_data_func,
                               rasterize_func, tile_def_class,
                               color_ranging_strategy='fullscan'):
    if color_ranging_strategy == 'fullscan':
        b = db.from_sequence(list(gen_super_tiles(full_extent, level, tile_def_class)))
...
def gen_super_tiles(extent, zoom_level, tile_def_class, span=None):
    xmin, ymin, xmax, ymax = extent
    super_tile_size = min(2**4 * 256,
                         (2 ** zoom_level) * 256)
    super_tile_def = tile_def_class(x_range=(xmin, xmax), y_range=(ymin, ymax), tile_size=super_tile_size)
...
def render_super_tile(tile_info, output_path, load_data_func,
                      rasterize_func, shader_func, post_render_func, tile_def_class):
    tile_size = tile_info['tile_size']
    ...
    return create_sub_tiles(ds_img, level, tile_info, output_path, post_render_func, tile_def_class)

def create_sub_tiles(data_array, level, tile_info, output_path, post_render_func, tile_def_class):

    # validate / createoutput_dir
    _create_dir(output_path)

    # create tile source
    tile_def = tile_def_class(x_range=tile_info['x_range'],
                                      y_range=tile_info['y_range'],
                                      tile_size=256)
...
def render_tiles(full_extent, levels, load_data_func,
                 ...
                 post_render_func, output_path, tile_def_class=MercatorTileDefinition, color_ranging_strategy='fullscan'):
    ...
    for level in levels:
        ...
        span = calculate_zoom_level_stats(full_extent, level,
                                          load_data_func, rasterize_func, tile_def_class,
                                          color_ranging_strategy='fullscan')

        super_tiles = list(gen_super_tiles(full_extent, level, tile_def_class, span))
        ...
        b.map(render_super_tile, output_path, load_data_func, rasterize_func, shader_func, post_render_func, tile_def_class).compute()
